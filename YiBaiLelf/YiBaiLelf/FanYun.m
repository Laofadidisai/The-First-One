...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        for (int i = 0; i<10; i++)
        {
            ...
            wind.frame = CGRectMake(arc4random()%200+50, 200, 2.4, 1);
            //最开始创建的雪花都在屏幕外面
            wind.isUsed = NO;
        ...
        dnowTimer = [NSTimer scheduledTimerWithTimeInterval:0.6 target:self selector:@selector(windDown) userInfo:nil repeats:YES];
...
-(void)findDnow
{
    for (int i = 0; i<windArray.count; i++)
    {
        ...
        {
            windyun.frame = CGRectMake(windyun.frame.origin.x-2, windyun.frame.origin.y-4, windyun.frame.size.width+2.4, windyun.frame.size.height + 1);
            if (windyun.frame.origin.x<-100)
            {
                windyun.isUsed = NO;
                windyun.frame = CGRectMake(arc4random()%200+50, 200, 2.4, 1);
